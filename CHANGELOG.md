...
* Default the Core plugin's new "Atmosphere Quality" Option to Reduced on the web and on integrated graphics, where the limb shell is most of the frame in any view with air; a discrete GPU keeps Normal. Note that every Compatibility session takes Reduced, the GL driver reporting no adapter type for the test to read.
